...
async def test_preprocess_returns_output_path(
    preprocessor: AudioPreprocessor, cache_dir: Path
) -> None:
    """Returns cache_dir/{guid}.mono.m4a on success."""
    with patch("components.audio_preprocessor.Ffmpeg") as mock_cls:
        mock_cls.return_value.run = AsyncMock(return_value=None)
        result = await preprocessor.preprocess(GUID, INPUT_PATH, DURATION)

    assert result == cache_dir / f"{GUID}.mono.m4a"


async def test_preprocess_creates_cache_dir(
    preprocessor: AudioPreprocessor, cache_dir: Path
) -> None:
    ...
    with patch("components.audio_preprocessor.Ffmpeg") as mock_cls:
        mock_cls.return_value.run = AsyncMock(return_value=None)
        await preprocessor.preprocess(GUID, INPUT_PATH, DURATION)

    assert cache_dir.is_dir()  # noqa: ASYNC240


async def test_preprocess_correct_ffmpeg_args(
    preprocessor: AudioPreprocessor, cache_dir: Path
) -> None:
...
async def test_preprocess_no_progress_callback(preprocessor: AudioPreprocessor) -> None:
    """on_progress=None works without error; Ffmpeg.run is still called."""
    with patch("components.audio_preprocessor.Ffmpeg") as mock_cls:
        mock_cls.return_value.run = AsyncMock(return_value=None)
        result = await preprocessor.preprocess(GUID, INPUT_PATH, DURATION, on_progress=None)

    assert result == preprocessor._cache_dir / f"{GUID}.mono.m4a"


async def test_preprocess_progress_start_and_end(
    ...
    with patch("components.audio_preprocessor.Ffmpeg") as mock_cls:
        ...
        await preprocessor.preprocess(GUID, INPUT_PATH, DURATION, on_progress=cb)

    assert (GUID, 0.0) in calls
    assert (GUID, 1.0) in calls


async def test_preprocess_progress_intermediate_forwarded(
    preprocessor: AudioPreprocessor,
) -> None:
...
async def test_preprocess_always_passes_callable_to_ffmpeg(
    preprocessor: AudioPreprocessor,
) -> None:
    """Ffmpeg.run() always receives a callable on_progress, even when preprocess gets None."""
    with patch("components.audio_preprocessor.Ffmpeg") as mock_cls:
        mock_run = AsyncMock(return_value=None)
        mock_cls.return_value.run = mock_run
        await preprocessor.preprocess(GUID, INPUT_PATH, DURATION, on_progress=None)

    call_kwargs = mock_run.call_args[1]
    assert callable(call_kwargs.get("on_progress"))


async def test_preprocess_raises_on_ffmpeg_error(
    preprocessor: AudioPreprocessor,
) -> None:
    """FfmpegError propagates out of preprocess()."""
    with patch("components.audio_preprocessor.Ffmpeg") as mock_cls:
...
async def test_preprocess_ffmpeg_error_does_not_call_progress(
    preprocessor: AudioPreprocessor,
) -> None:
    """When FfmpegError is raised, on_progress is not called."""
    cb = AsyncMock()
...
async def test_preprocess_logs_error_on_ffmpeg_failure(
    preprocessor: AudioPreprocessor,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """ERROR log contains the guid; ffmpeg stderr is NOT in the ERROR log."""
    with patch("components.audio_preprocessor.Ffmpeg") as mock_cls:
...
async def test_preprocess_logs_stderr_at_debug_on_ffmpeg_failure(
    preprocessor: AudioPreprocessor,
    caplog: pytest.LogCaptureFixture,
) -> None:
...
async def test_preprocess_logs_debug_on_success(
    preprocessor: AudioPreprocessor,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """A debug log is emitted for a successfully preprocessed episode."""
    with patch("components.audio_preprocessor.Ffmpeg") as mock_cls:
        mock_cls.return_value.run = AsyncMock(return_value=None)
        with caplog.at_level(logging.DEBUG, logger="components.audio_preprocessor"):
            await preprocessor.preprocess(GUID, INPUT_PATH, DURATION)
